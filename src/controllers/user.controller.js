...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Somethin went wrong while genrating refresh and access token"
    );
  }
};
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  //  Hold old avatar public_id in variable
  let oldAvatarPublicId = null;

  if (existingUser.avatar) {
    const oldUrl = existingUser.avatar;

    oldAvatarPublicId = oldUrl.split("/").pop().split(".")[0];
  }
